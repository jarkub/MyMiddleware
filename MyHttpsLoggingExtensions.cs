...
public static /*partial*/ class MyHttpsLoggingExtensions
{
    ...
    private static readonly Action<ILogger, Int32, Exception?> __MyPortLoadedFromConfigCallback =
        LoggerMessage.Define<Int32>(LogLevel.Debug, new EventId(2, "PortLoadedFromConfig"), "Https port '{port}' loaded from configuration.", new LogDefineOptions() { SkipEnabledCheck = true });
    ...
    private static readonly Action<ILogger, Int32, Exception?> __MyPortFromServerCallback =
        LoggerMessage.Define<Int32>(LogLevel.Debug, new EventId(5, "PortFromServer"), "Https port '{httpsPort}' discovered from server endpoints.", new LogDefineOptions() { SkipEnabledCheck = true });

    public static /*partial*/ void MyPortFromServer(this ILogger logger, Int32 httpsPort)
    {
        if (logger.IsEnabled(LogLevel.Debug))
        {
