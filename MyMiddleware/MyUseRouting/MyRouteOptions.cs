...
#endif
{
...
#if !COMPONENTS
    private ICollection<MyEndpointDataSource> _endpointDataSources = default!;

    /// <summary>
    /// Gets a collection of <see cref="EndpointDataSource"/> instances configured with routing.
    /// </summary>
    internal ICollection<MyEndpointDataSource> EndpointDataSources
    {
        get
        {
    ...
    {
        [RequiresUnreferencedCode($"The linker cannot determine what constraints are being added via the ConstraintMap property. Prefer {nameof(MyRouteOptions)}.{nameof(SetParameterPolicy)} instead for setting constraints. This warning can be suppressed if this property is being used to read or delete constraints.")]
        get
        {
...
#if !COMPONENTS
        AddConstraint<MyRegexErrorStubRouteConstraint>(defaults, "regex"); // Used to generate error message at runtime with helpful message.
