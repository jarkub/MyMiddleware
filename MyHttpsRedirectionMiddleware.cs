...
public class MyHttpsRedirectionMiddleware
{
    ...
    /// <summary>
    /// Initializes <see cref="MyHttpsRedirectionMiddleware" />.
    /// </summary>
    ...
    public MyHttpsRedirectionMiddleware(RequestDelegate next, IOptions<HttpsRedirectionOptions> options, IConfiguration config, ILoggerFactory loggerFactory)
    {
        ...
        _logger = loggerFactory.CreateLogger<MyHttpsRedirectionMiddleware>();
    }
    ...
    public Task Invoke(HttpContext context)
    {
        ...
        _logger.MyRedirectingToHttps(redirectUrl);
    ...
    private int TryGetHttpsPort()
    {
        ...
        if (nullablePort.HasValue)
        {
            ...
            _logger.MyPortLoadedFromConfig(port);
            return port;
        }

        if (_serverAddressesFeature == null)
        {
            _logger.MyFailedToDeterminePort();
            return PortNotFound;
        }
