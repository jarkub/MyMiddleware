...
using razormy.MyMiddleware.Routing.Matching;
using razormy.MyMiddleware.Routing.ShortCircuit;

//namespace Microsoft.AspNetCore.Routing;

namespace razormy.MyMiddleware.Routing;

internal sealed partial class MyEndpointRoutingMiddleware
{
    private const string DiagnosticsEndpointMatchedKey = "Microsoft.AspNetCore.Routing.EndpointMatched";

    private readonly MyMatcherFactory _matcherFactory;
    private readonly ILogger _logger;
    private readonly MyEndpointDataSource _endpointDataSource;
    private readonly DiagnosticListener _diagnosticListener;
    private readonly MyRoutingMetrics _metrics;
    private readonly RequestDelegate _next;
    private readonly MyRouteOptions _routeOptions;
    private Task<MyMatcher>? _initializationTask;

    public MyEndpointRoutingMiddleware(
        MyMatcherFactory matcherFactory,
        ILogger<MyEndpointRoutingMiddleware> logger,
        ...
        RequestDelegate next)
    {
        ...
        _endpointDataSource = new MyCompositeEndpointDataSource(endpointRouteBuilder.DataSources);
    }
    ...
    public Task Invoke(HttpContext httpContext)
    {
        ...
        if (endpoint != null)
        {
            MyLog.MatchSkipped(_logger, endpoint);
        ...
        static async Task AwaitMatcher(MyEndpointRoutingMiddleware middleware, HttpContext httpContext, Task<MyMatcher> matcherTask)
        {
        ...
        static async Task AwaitMatch(MyEndpointRoutingMiddleware middleware, HttpContext httpContext, Task matchTask)
        {
            await matchTask;
            await middleware.SetRoutingAndContinue(httpContext);
        }
    ...
    private Task SetRoutingAndContinue(HttpContext httpContext)
    {
        ...
        else
        {
            ...
            {
                var isFallback = endpoint.Metadata.GetMetadata<MyFallbackMetadata>() is not null;

                MyLog.MatchSuccess(_logger, endpoint);

                if (isFallback)
                {
                    MyLog.FallbackMatch(_logger, endpoint);
                }
            ...
            var shortCircuitMetadata = endpoint.Metadata.GetMetadata<MyShortCircuitMetadata>();
    ...
    private Task ExecuteShortCircuit(MyShortCircuitMetadata shortCircuitMetadata, Endpoint endpoint, HttpContext httpContext)
    {
        ...
        if (!_routeOptions.SuppressCheckForUnhandledSecurityMetadata)
        {
            ...
            if (endpoint.Metadata.GetMetadata<IAntiforgeryMetadata>() is { RequiresValidation: true } &&
                ...
                MyHttpExtensions.IsValidHttpMethodForForm(method))
            {
        ...
        {
            MyLog.ShortCircuitedEndpoint(_logger, endpoint);
        }
        return Task.CompletedTask;
    }
    ...
    private Task<MyMatcher> InitializeAsync()
    {
    ...
    private Task<MyMatcher> InitializeCoreAsync()
    {
        var initialization = new TaskCompletionSource<MyMatcher>(TaskCreationOptions.RunContinuationsAsynchronously);
    ...
    private void SetMaxRequestBodySize(HttpContext context)
    {
        ...
        {
            MyLog.RequestSizeLimitMetadataNotFound(_logger);
            return;
        }
        ...
        {
            MyLog.RequestSizeFeatureNotFound(_logger);
        }
        else if (maxRequestBodySizeFeature.IsReadOnly)
        {
            MyLog.RequestSizeFeatureIsReadOnly(_logger);
        }
        else
        {
            ...
            {
                MyLog.MaxRequestBodySizeSet(_logger,
                    maxRequestBodySize.Value.ToString(CultureInfo.InvariantCulture));
            }
            else
            {
                MyLog.MaxRequestBodySizeDisabled(_logger);
            }
        }
    }
...
public static partial class MyLog
{
    ...
    [LoggerMessage(1, LogLevel.Debug, "Request matched endpoint '{EndpointName}'", EventName = "MatchSuccess")]
    internal static partial void MatchSuccess(ILogger logger, string? endpointName);
    ...
    [LoggerMessage(3, LogLevel.Debug, "Endpoint '{EndpointName}' already set, skipping route matching.", EventName = "MatchingSkipped")]
    internal static partial void MatchingSkipped(ILogger logger, string? endpointName);
